-- 20260930_sons_position_check.sql
-- Actualiza el diagnóstico del superadmin para incluir la columna posicion
-- del setlist (migración 20260929_service_songs_position.sql).
-- Ejecutar solo si ya aplicaste 20260922_migrations_diagnostics.sql.
...
as $$
  select * from (values
    ...
    ('songs_position', 'Setlist ordenable (posicion)', exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'service_songs' and column_name = 'posicion'), '20260929_service_songs_position.sql')
